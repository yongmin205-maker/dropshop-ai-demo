...
P0 출시 단계는 **Stage 0 — Daily Pull 1회 (03:00 America/New_York)**. (원래 11:00+22:00 2회을 제안했으나 2026-05-14 사용자 피드백으로 1회로 단순화 — NYC 매장은 새벽이 무조건 닫혀있고, 24h 윈도우에 어제 하루치가 완전히 들어감). webhook은 의도적으로 도입 안 함. 이유는 단순함이다:
...
| **C-0. Daily Pull 1회 (Stage 0, 03:00 ET)** | T-24h max | **가장 낮음** | ✅ 페이지네이션 1회/24h | ✅ **P0 채택** |
| C-1. Webhook + Nightly Snapshot | ≤ 1초 / 누락분 ≤ 24h | 중간 | ✅ | Stage 1 (P1) |
| D. CDC stream | 실시간 | 높음 | — | 자체 POS 갔을 때만 가능 |

C-0이 P0인 이유: **신선도 손실(최대 24시간) < 구현 단순함의 이득 + dependency 제거**. 그리고 신선도 자체가 점주에게 *치명적인 경우가 없다* — 점주는 daily/weekly 패턴을 묻지, "방금 5분 전에 들어온 주문"을 묻지 않는다. (그런 lookup은 on-demand call로 처리.)
...
### C-0 (Daily Pull 1회) 자세히

매장 timezone은 **America/New_York** (NYC 드라이클리닝 매장). NYC 동네 드라이클리닝은 보통 19:00–20:00 마감, 새벽은 무조건 닫혀있음 → 03:00 ET가 API 부하 제로 구간.

```
[03:00 America/New_York — 하루 1회]
  └─ getOrders(dateFrom=어제 03:00, dateTo=지금) → upsert orders
  └─ getCustomer(updatedSecondsAgoFrom=86400) → upsert customers
  └─ getPayments(dateFrom=어제 03:00, dateTo=지금) → upsert payments
  └─ getProducts/getPriceLists 전체 스냅샷 → diff alert
```

작동 시나리오:
- 점주가 아침 09:00에 "어제 매출?" 묻는다 → 오늘 03:00 pull 결과 → 완전히 정확
- 점주가 점심 13:00에 "오늘 오전 어떠?" 묻는다 → mirror엔 어제까지만 있음 → fast-path tool `aggregateRevenueLive(today)`가 on-demand `getOrders(dateFrom=00:00 today)` 처리 → 5–10초
- 점주가 22:00에 "오늘 마감 주문 다 끝난 거 맞아?" 묻는다 → fast-path live API call → 정확

**핵심 원칙**: mirror는 *어제까지*의 진실. 오늘 데이터는 fast-path tool로 live API 호출. Owner Assistant가 "이 답은 오늘 03:00 ET까지의 데이터" 또는 "이 답은 방금 확인한 실시간 데이터"를 명시적으로 표시.
...
→ pull 1회당 ~5 API call, 일 1회 = 일 5 call = 월 150 call. 50K 한도의 0.3%. fast-path tool이 하루에 수십번 더 쓰이더라도 총 월 ~3K–5K call 수준 (10% 내).
...
- `0 3 * * *` America/New_York → `runDailyPull("03am_et")`

(매장 timezone은 NYC로 확인됨 — 2026-05-14 사용자 응답.)

### Overlap window (28h)

단일 pull이므로 **24h business day + 4h buffer**를 잡는다. 마감 직전 (예: 19:55)에 주문이 들어왔다가 주문 완성이 20:30까지 추가로 변경된 경우가 다음날 03:00 pull의 24h 윈도우에 안전히 들어와야 함. 이전 pull 실패 시의 self-healing을 위해 28h로 확장 (이는 upsert이므로 중복 무해). pull이 1회 멈추면 다음 날 03:00 pull이 28h 윈도우로 이전 누락분을 자동으로 쓸어담음.
...
| 실패 모드 | 감지 방법 | 대응 |
|---|---|---|
| 03:00 ET pull 실패 | sync_log 마지막 trigger row의 `finishedAt is null` | owner notify + 다음날 03:00 pull이 28h overlap window로 자동 복구 |
| 두 날 연속 pull 실패 | 48h 이상 새로운 finished sync 없음 | 더 시끄러운 alert (SMS via Phase 23f Nextiva) + admin UI banner. 이 경우 이틀을 넘기면 28h 윈도우도 일부 누락 가능 → 수동 backfill 권장 |
...
1. CleanCloud admin에 **data retention 설정** 페이지가 있는지? 있으면 스크린샷 한 장.
2. **B2B 비즈니스 계정** 운영 여부? — 있다면 `business_accounts` mirror 추가.
3. **Locker 서비스** 운영 여부? — 안 하면 locker 필드 분석 제외.

*(timezone은 NYC로 확정, 앱은 태블릿 POS 전용으로 확정 — 손님용 모바일 앱 아님 → push channel 비활성. Stage 0 범위에서 더 확인할 것 없음.)*
