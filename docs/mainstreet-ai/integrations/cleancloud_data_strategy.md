...
## 5. Mirror 대상 — 4개 코어 + 1개 메타

| 테이블 | source endpoint | 갱신 트리거 | 핵심 컬럼 |
|---|---|---|---|
| `cc_customers` | `getCustomer` (날짜 범위 페이지네이션 + webhook `customer.created/updated/deleted`) | webhook 실시간 + nightly full snapshot | `customerID PK`, 위 §2.1 ✅ 필드 13개, `raw_payload JSON`, `synced_at` |
| `cc_orders` | `getOrders` (날짜 범위 + webhook `order.*`) | webhook 실시간 + nightly diff (last 30 days) | `orderID PK`, `customerID FK`, 위 §2.2 ✅ 필드, `raw_payload JSON`, `synced_at` |
| `cc_order_items` | `getOrders` 응답의 `products[]` 펼침 | `cc_orders` 업데이트 시 자동 | `orderID FK`, `productID`, `name`, `price`, `pieces`, `quantity` |
| `cc_payments` | `getPayments` (날짜 범위) | nightly + on-demand | `paymentID PK`, `orderID FK`, `amount_cents`, `type`, `date`, `refunded`, `raw_payload JSON` |
| `cc_products` | `getProducts` / `getPriceLists` (전체) | nightly snapshot + diff alert | `productID PK`, `priceListID`, `name`, `price_cents`, `category`, `synced_at` |
| **`cc_sync_log`** (메타) | — | 모든 sync 작업 | `sync_id PK`, `endpoint`, `started_at`, `finished_at`, `rows_synced`, `error`, `webhook_event_id FK?` |

`cc_sync_log`가 별도 있어야 하는 이유: Owner Assistant가 "이 답변 얼마나 fresh해?"라고 답할 수 있어야 함. 우리가 시계열 답변을 하면서 "데이터는 3시간 전까지 반영" 같은 메타 표시 가능.

스키마 detail은 24b 문서에서 확정.
...
5. CleanCloud admin → API/Webhooks 페이지에서 **활성화된 webhook 8개의 정확한 이름** 캡쳐. (Phase 23f-7 webhook handler가 이미 8개 모두 지원하지만, 어느 게 실제 점등되는지 확인 필요)
