...
| Tool | 카테고리 | 입력 | 출력 |
|---|---|---|---|
| `findCustomerByPhoneOrName` | 1 | `query: string` | `customer` (full row, from mirror `customers`) |
| `getCustomerRecentOrders` | 1 | `customerId, limit=10` | `orders[]` (mirror `orders`) |
| `getOrderDetails` | 1 | `orderId` | `order + items[]` (mirror, fallback to live API if not found) |
