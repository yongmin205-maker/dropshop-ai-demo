...
Lead pass merged sub-agent A (correctness) / B (robustness) / C (tests).
Full report: `docs/mainstreet-ai/reviews/phase25v_backfill_review.md`. Tests:
`pnpm test --run server/integrations/cleancloud/backfill.test.ts` 15/15 green
(8 existing + 7 new); full suite 512 passing (+7 net new). `pnpm exec tsc
--noEmit` clean.

Applied to `backfill.ts` / `backfill.test.ts` / `cleanCloudTransport.ts`:
- [x] Agent A-F1 — `monthsBack` off-by-one: dropped the `-1` in `startOfMonthUTC`. `monthsBack=12` now genuinely covers 12 full prior calendar months + current partial.
- [x] Agent A-F3 — Empty `getCustomer` envelope → `[{}]` synthesis: transport's empty-envelope fallback now returns `[]` instead of `{}`. Stops fake `rowsFetched: 1` for empty windows.
- [x] Agent A-F4 — Bisection depth comment lie: `DEFAULT_MAX_BISECT_DEPTH` lowered to 4 with honest comment (the > 6h floor makes depth 5 unreachable from a 7d start).
- [x] Agent A-F5 — Backfill now uses `excludeDeactivated: 0` so historical orders' `customerExternalId` references resolve to a customer row. Daily pull unchanged at `1`.
- [x] Agent B-F5 — Bisection parent row no longer carries `error: "bisecting..."`. Finishes with `rowsFetched: 0, rowsUpserted: 0`; admin UI's `error != null` filter now matches only true failures.
- [x] Agent B-F6 — Backfill-local 1-retry on `^CleanCloud HTTP 5\d\d` with 2s backoff. Transport untouched per constraint.
- [x] Agent B-F8 — `console.info` at entry/per-window-success/exit, `console.warn` on bisection + 5xx retry, `console.error` on terminal failure.
- [x] Agent C #1 — `it("hits maxBisectDepth and records the cap error as a final failure")`. Closes existing test #6 theatre.
- [x] Agent C #2 — `it("walks 6 months of windows with the expected post-fix counts")` (7 customer + 29 order windows starting 2025-11-01).
- [x] Agent C #3 — `it("survives getCustomer returning a single object instead of an array")`.
- [x] Agent C #4 — `it("records the DB error on the failing window and continues subsequent windows")`.
- [x] Agent C #5 — `it("counts only post-filter rows in summary.customers.upserted (nulls excluded)")`.
- [x] Agent C #6 — `it("startOfMonthUTC correctly handles year-boundary straddles")` — post-A-F1-fix expected 2025-02-01.
- [x] Agent C #7 — `it("returns no windows when from > to (inverted range)")`.
- [x] Existing tests #4 + #6 renamed/strengthened — #4 now asserts 30-day customer width (claim previously lied); #6 renamed to honestly describe "non-cap hard error" path and asserts no bisection recursion fires.

Deferred (these are the blockers that need a follow-up branch, NOT applied here):
- [ ] **Agent B-F1 + B-F2 — Heartbeat-job migration.** 12-month backfill in a single Cloud Run request still cannot reliably complete in 180s. Defensive `console.warn` ships when `monthsBack > 6` but no hard cap. Follow-up: implement `/api/scheduled/cleancloud-backfill` heartbeat that picks up `posSyncLog` rows with `endpoint = "backfill_root"` and processes one week-window per cron tick. Tracking as Phase 25-verify-followup.
- [ ] **Agent B-F3 — Module-level rate limiter doubles if Cloud Run autoscales to ≥2 instances.** Follow-up: deploy-config — pin `maxInstances=1` for this service. Not a code change.

Deferred as nits (low value, not for this PR):
- [ ] Agent A-F2 — `rollingWindows` boundary instant shared between adjacent windows. Harmless today (idempotent upserts).
- [ ] Agent C #8 — Concurrent `runBackfill` invocations. Per Agent C, this is a router-layer concern, not a backfill-layer one.
