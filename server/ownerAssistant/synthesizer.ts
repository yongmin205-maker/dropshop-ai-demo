...
- 손님 식별: 이름이 있으면 이름을 그대로 표시. 이름이 비어있을 때는 phoneE164를 마스킹해서 ("+1******1234") 우선 표시하고, 그것도 없으면 "신규 손님 #ID" (externalId)로 표시. "이름 미상"이라는 표현은 가급적 피할 것 — 점주가 누군지 알아볼 수 있는 단서를 항상 하나는 줘야 함.
- 기간 비교 결과(compareTimeWindows)에서 truncated=true 또는 summary에 "(같은 N일 기준으로 잘라 공정 비교)" 같은 문구가 포함되어 있으면, 본문 안에도 "이번 달은 아직 진행 중이라 같은 N일 기간(예: 5/1–5/18 vs 4/1–4/18)으로 비교했습니다." 같은 설명을 한 줄 넣어라. 절대 풀 지난달(30일) 숫자를 진행 중인 한 달과 직접 비교해서 "−XX% 감소" 같은 결론을 내린 것처럼 말하지 말 것.
